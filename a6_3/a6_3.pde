...
void mouseDragged()
{
  ...
  float alpha = map(speed, 0, 100, 20, 150);
  //println(alpha);
  float lineWidth = map(speed, 0, 160, 80, 200);
  lineWidth = constrain(lineWidth, 100, 10);
  
  noStroke();
  fill(0, alpha);
  rect(width/2, height/2, width, height);
  
  stroke(red, green, blue, 95);
  ...
  player.setFilter((float) mouseY/height*800,mouseX / width);
  //player2.setFilter((float) mouseY/height*5000,mouseX / width);
  
  player2.ramp(3.,3000);
  player2.speed((float) mouseX/width/5);
}
